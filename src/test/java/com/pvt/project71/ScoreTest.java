...
public class ScoreTest {
    ...
    void setUp() {
        ...
        event.getAdminUsers().add(user);
        event = eventService.save(event, user);
    }
...
//                .andExpectAll(MockMvcResultMatchers.status().isOk()
//                , MockMvcResultMatchers.jsonPath("$[3]").exists(),
//                MockMvcResultMatchers.jsonPath("$[4]").doesNotExist());
//    }
    @Test
    public void testDeletingEventDeletesScoreToo() throws Exception {
        ScoreEntity validScoreEntity = TestDataUtil.createValidScoreEntity(user,event);
        scoreRepository.save(validScoreEntity);
        eventService.delete(event.getId(), user);
        assertFalse(scoreService.findAllByEvent(event.getId()).isPresent());
    }
    @Test
    public void testDeletingUserDeletesScoreInEventsList() throws Exception {
        ScoreEntity validScoreEntity = TestDataUtil.createValidScoreEntity(user,event);
        scoreRepository.save(validScoreEntity);
    ...
    public void testCreateValidScoreHttpCreated() throws Exception {
        ScoreEntity validScoreEntity = TestDataUtil.createValidScoreEntity(user,event);
        mockMvc.perform(MockMvcRequestBuilders.post("/events/" + validScoreEntity.getScoreId().getEvent().getId()
                        + "/join").with(jwt().jwt(getUserToken())))
                .andExpect(MockMvcResultMatchers.status().isCreated());
    }
    @Test
    public void testCreateValidScoreAgainGivesConflict() throws Exception {
        ScoreEntity validScoreEntity = TestDataUtil.createValidScoreEntity(user,event);
        mockMvc.perform(MockMvcRequestBuilders.post("/events/" + validScoreEntity.getScoreId().getEvent().getId()
                                + "/join").with(jwt().jwt(getUserToken())));
        mockMvc.perform(MockMvcRequestBuilders.post("/events/" + validScoreEntity.getScoreId().getEvent().getId()
                + "/join").with(jwt().jwt(getUserToken()))).andExpect(MockMvcResultMatchers.status().isConflict());
    }
    ...
    @Test
    public void testDeletingUserDeletesScoreToo() throws Exception {
        ScoreEntity validScoreEntity = TestDataUtil.createValidScoreEntity(user,event);
        scoreRepository.save(validScoreEntity);
        userService.delete(user.getEmail());
        assertFalse(scoreService.findAllByUser(user).isPresent());
    }
